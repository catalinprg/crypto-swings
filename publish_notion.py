"""Publish a markdown briefing to Notion as a child page under the active
asset's Swings parent page.

Usage: ASSET=btc python3 publish_notion.py <path_to_briefing.md> <TIMESTAMP>
       ASSET=eth python3 publish_notion.py <path_to_briefing.md> <TIMESTAMP>

Requires env vars:
  ASSET         btc | eth (selects parent page from config/{asset}.json)
  NOTION_TOKEN  Notion Internal Integration Token

The parent page for the selected asset must be shared with the integration.
...
PARENT_PAGE_ID = CONFIG.notion_parent_id
...
def main():
    if not NOTION_TOKEN:
        ...
        sys.exit(1)
    if len(sys.argv) != 3:
        print("usage: publish_notion.py <briefing.md> <TIMESTAMP>", file=sys.stderr)
        sys.exit(2)

    md_path, timestamp = sys.argv[1], sys.argv[2]
    ...
    page_id, page_url = create_page(timestamp, blocks[:BATCH])
